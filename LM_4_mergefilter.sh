#!/bin/bash --login
#SBATCH -o batch_LM_4.%A.out.txt
#SBATCH -e batch_LM_4.%A.err.txt
#SBATCH --ntasks=10
...
previous_step="$dir2/QB_LM_3B_Progress.txt";
this_step="$dir2/QB_LM_4_Progress.txt";
...
woof=$(perl ~/scripts/Linkage_Mapping/list_complete.pl $qx_names $previous_step Step_3B_Complete);
if [[ $woof != "ALL_COMPLETE" ]]; then echo "Not all complete" && exit; fi;  

truncate -s 0 $dir2/$name.bcfs_tomerge.$dat;
while read file; do echo $dir2/$file.bam.DP7.bcf.gz.norm >> $dir2/$name.bcfs_tomerge.$dat; done < $qx_names;
...
echo "Started $dat $name" >> $this_step &&
	bcftools merge --gvcf $genome --threads 10 -Ob -o $dir2/$name.merged_snps.bcf.gz -l $dir2/$name.bcfs_tomerge.$dat && bcftools view $dir2/$name.merged_snps.bcf.gz -Ov -o $dir2/$name.merged_snps.vcf && 
      bcftools plugin setGT $dir2/$name.merged_snps.vcf -- -t q -n . -i "FMT/DP<7 | FMT/GQ<20 | QUAL<15 | (FMT/GQ > 0 & ((DP4[2]+DP4[3])/(DP4[0]+DP4[1])<0.33333))"  |  bcftools view -i 'F_MISSING<0.2' -m2 -M2 -v snps  -Ob -o $dir2/$name.merged_snps_lowmem.filter2.bcf.gz && bcftools view -Ov -o $dir2/$name.merged_snps_lowmem.filter2.vcf $dir2/$name.merged_snps_lowmem.filter2.bcf.gz &&
      echo "$name $dat Finished" >> $this_step;
